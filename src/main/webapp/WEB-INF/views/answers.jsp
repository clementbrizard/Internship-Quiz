...
<div class="container col-md-12">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <form id="deleteForm/${item.id}" method="POST"
                              action="${contextPath}/answers/manage/delete/${item.id}">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                        </form>
                        <form id="editForm/${item.id}" method="POST"
                              action="${contextPath}/answers/manage/edit/${item.id}">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                        </form>
                        <form id="disableForm/${item.id}" method="POST" action="${contextPath}/answers/manage/disable/${item.id}">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                        </form>
    ...
    <div class="col-md-12 text-center">
        ...
        <form id="addForm" method="POST"
              action="${contextPath}/answers/manage/add">
            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
        </form>
...
<script src="${contextPath}/resources/js/manage_answer.js"></script>
